...
first_date <- min(as.POSIXct(lull$date))
last_date <- max(as.POSIXct(lull$date))
...
lull_table<-kable(combined_lull_counts, format = "markdown", caption="Table 2. Quality control flag count summary for Fulmore River.") 
...
first_date <- min(as.POSIXct(full$date))
last_date <- max(as.POSIXct(full$date))
...
first_date <- min(as.POSIXct(heyd$date))
last_date <- max(as.POSIXct(heyd$date))
